...
bool Model :: update()
{
  ...
  for (std::list<GameObject*>::iterator it = active_ptrs->begin(); it != active_ptrs->end(); it++)
    {
      //for calling the specific type of update function for that object
      if(dynamic_cast<Sailor*>(*it)!=NULL)
	{
	  if(dynamic_cast<Sailor*>(*it)->display_code=='M')
	    {
	      cout<<"call merchant update"<<endl;
	      check = dynamic_cast<Merchant*>(*it)->update();
	  cout<<check<<endl;
	    }
	  if((*it)->display_code=='R')
	    {
	      cout<<"pirate updaet"<<endl;
	      check = dynamic_cast<Pirate*>(*it)->update();
 cout<<check<<endl;
	    }
	}
      if(dynamic_cast<Port*>(*it)!=NULL)
	{
	  cout<<"port update"<<endl;
	  check = dynamic_cast<Port*>(*it)->update();
 cout<<check<<endl;
	}
      if(dynamic_cast<Dock*>(*it)!=NULL)
	{
	  cout<<"dock update"<<endl;
	  check = dynamic_cast<Dock*>(*it)->update();
 cout<<check<<endl;
	}
      if (check)//if the object was updated output is true
	{
...
for (std::list<GameObject*>::iterator it = active_ptrs->begin(); it != active_ptrs->end(); it++)
    {
      if(dynamic_cast<Dock*>(*it)!=NULL)
	{
	  if(dynamic_cast<Dock*>(*it)->get_id()==object->get_id())
	    throw InvalidInput("ID already in use for this type");
	}
      else
	{
	  if(dynamic_cast<Port*>(*it)!=NULL)
	    {
	      if(dynamic_cast<Port*>(*it)->get_id()==object->get_id())
		throw InvalidInput("ID already in use for this type");
	  ...
	  else
	    {
	      if(dynamic_cast<Sailor*>(*it)!=NULL)
		{
		  if(dynamic_cast<Sailor*>(*it)->get_id()==object->get_id())
		    throw InvalidInput("ID already in use for this type");
		}
	    }
	}
