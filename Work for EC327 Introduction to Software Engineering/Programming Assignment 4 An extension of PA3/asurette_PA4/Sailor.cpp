...
Sailor :: Sailor(char dispCode, Model* world):GameObject(dispCode, 0)//default constructor
{
  this->world=world;
...
Sailor::Sailor(char dispCode, int in_id, Dock* hideout, Model* world) : GameObject(dispCode, in_id, hideout->get_location())//constructor
{
  this->world=world; 
...
//need help with the new constructor, to get around pirates having a null as their hideout
Sailor::Sailor(char display_code, int in_id, CartPoint location, Model* world): GameObject(display_code, in_id, location)
{
  this->world=world;
...
bool Sailor::is_hidden()
 {
   ...
   if((cart_compare(this->get_location(), hideout->get_location()))) //need these to be cartpoints
     {
...
void Sailor::show_status()
{
  ...
  cout<<display_code<<" with ID "<<get_id()<<" at location "<<this->location<<" ";
  ...
  switch (state)
	...
	{
	  //  cout<<endl;//used to fix a line error
	  break;
	}
      }
    }
...
/*--------------------------------------------------------------------------------------
  -------------------------------------------------------------------------------------*/
void Sailor::start_plunder(Sailor* sailor)
{
...
bool Sailor::start_recruiting(Sailor* sailor)
{
...
{
  double as = static_cast<double>(attack_strength);

  cout<<as<<endl;

  double hp = this->get_health();
...
void Sailor::set_health(double hp)
{
...
void Sailor::add_health(double hp)
{
